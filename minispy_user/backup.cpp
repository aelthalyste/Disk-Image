...
SetupFullOnlyStream(wchar_t Letter, DotNetStreamInf *SI, bool ShouldCompress, bool RegionLock){
    ...
    if(true == Success){
        ...
        if(SI){
            ...
            GenerateMetadataName(Result.BackupID, NAR_FULLBACKUP_VERSION, SI->MetadataFileName);
            SI->FileName = L"";
            GenerateBinaryFileName(Result.BackupID, NAR_FULLBACKUP_VERSION, SI->FileName);
        }
...
GetMFTandINDXLCN(char VolumeLetter, HANDLE VolumeHandle) {
    ...
    uint32_t MEMORY_BUFFER_SIZE = 1024LL * 1024LL * 16;
    uint32_t ClusterExtractedBufferSize = 1024 * 1024 * 4;
...
SaveMetadata(char Letter, int Version, int ClusterSize, BackupType BT,
    ...
    if (ClusterSize <= 0 || ClusterSize % 512 != 0) {
        printf("SAVEMETADATA : INVALID CLUSTER SIZE %d\n", ClusterSize); 
        return FALSE;
    }
    ...
    BM.MetadataInf.MagicNumber = GlobalBackupMetadataMagicNumber;
    ...
    
    {
        ...
        for(uint64_t mi =0; mi<MFTRecordLen; mi++){
            int32_t SFR = NarSetFilePointer(VSSHandle, (uint64_t)MFTRecords[mi].StartPos*(uint64_t)ClusterSize);
            ASSERT(!!SFR);
        ...
        if(ReadFile(FileHandle, &Metadata, sizeof(Metadata), &BytesOperated, 0) && BytesOperated == sizeof(Metadata)){
            ...
            if(NarSetFilePointer(FileHandle, FILE_BEGIN)){
                if(WriteFile(FileHandle, &Metadata, sizeof(Metadata), &BytesOperated, 0) && BytesOperated == sizeof(Metadata)){
                    ...
                    printf("Couldnt update metadata task name and task description(%S), err %d\n", FileName, GetLastError());
                }
            }
        }
        else{
            printf("Couldn't read %i bytes from file %S, task was NarEditTaskNameAndDescription, err %d\n", &BytesOperated, FileName, GetLastError());
        }
    }
    else{
