...
SetIncRecords(volume_backup_inf* VolInf) {
  ...
  printf("Before set file pointer\n");
...
OfflineRestore(restore_inf* Inf) {
  ...
  /*
NOTE(Batuhan): Volume handle and stream structure will be available for Offline##Restore functions, since there are another steps required to complete restore operation, diff and incremental backup functions must take care of rest of the operation.
   */

  std::wstring a = GenerateFBFileName(Inf->TargetLetter).c_str();
