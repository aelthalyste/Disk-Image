...
*/
region_chain*
InitDBMetadataChain(nar_record* R, UINT32 C) {
	...
	for (int i = 0; i < C; i++) {//First element already added
		AppendToList(Chain, R[i]);
	}
...
void
DeleteChain(region_chain* Chain) {
	Chain = GetListHead(*Chain);
	region_chain* N;
	while (Chain) {
		N = Chain->Next;
		CLEANMEMORY(Chain);
...
D_Cleanup:
	...
	if (!ErrorOccured) {
		...
		printf("No error occured, updating metadata\n");
		if (SetFilePointer(VolInf->LogHandle, 0, 0, FILE_BEGIN) != 0) {
			printf("Cant set file pointer to beginning of the file\n");
		}
		WriteFile(VolInf->LogHandle, DiffRecords.Data, DiffRecords.Count * sizeof(nar_record), 0, 0);
		printf("New log file size must be -> %I64d\n", DiffRecords.Count * sizeof(nar_record));
		if (!SetEndOfFile(VolInf->LogHandle)) {
