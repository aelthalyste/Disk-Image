...
inline std::string
GenerateFBMetadataFileName(char Letter) {
  std::string Return = "FMetadata_";
  Return += Letter;
  return Return;
}
...
#if 0
  // NOTE(Batuhan) : Experimental
  printf("Checking if OS volume\n");
  if (Inf->IsOsVolume || Inf->SrcLetter == 'C') {
    printf("Experimental boot partition restore will be done\n");
    Inf->IsOsVolume = 1;
    file_read data = NarReadFile(GenerateFBMetadataFileName((char)Inf->SrcLetter).c_str());
    data_array<nar_record> temp;
    
    temp.Count = data.Len / sizeof(nar_record);
    temp.Data = (nar_record*)data.Data;
    printf("record count => %i\n", temp.Count);
    ULONGLONG SizePure = ((ULONGLONG)temp.Data[temp.Count - 1].StartPos + (ULONGLONG)temp.Data[temp.Count - 1].Len) * Inf->ClusterSize;
    ULONGLONG SizeMB = SizePure / (1024 * 1024);
    printf("Calculating EFI file size\n");
    printf("Fullbackup volume size(MB) %I64d\n", SizeMB);
    HANDLE EFIHANDLE = CreateFileW(GenerateSystemPartitionFileName(Inf->SrcLetter).c_str(), GENERIC_READ, 0, 0, OPEN_EXISTING, 0, 0);
    if (EFIHANDLE != INVALID_HANDLE_VALUE) {
      ULONGLONG EfiSizeMB = GetFileSize(EFIHANDLE, 0) / (1024 * 1024);
      NarCreateGPTBootPartition(Inf->DiskID, 31370, EfiSizeMB, 0, (char)Inf->TargetLetter);
      //31370
      

    }
    else {
      printf("Can't open efi file\n");
      DisplayError(GetLastError());
    }
    CloseHandle(EFIHANDLE);
  }
#endif 
 Inf->DiskID = 1;
 RestoreSystemPartitions(Inf);
  ...
  if (FBMetadata != INVALID_HANDLE_VALUE) {
    ...
    if (ReadFile(FBMetadata, FullRecords.Data, FileSize, &BytesRead, 0) && BytesRead == FileSize) {
      ...
      ULONGLONG VolumeSize4KB = FullRecords.Data[FullRecords.Count - 1].StartPos + FullRecords.Data[FullRecords.Count - 1].Len;
      //TODO reverse list
      ULONGLONG VolumeSizeMB = VolumeSize4KB / (1024 * 4);
      //NarSetVolumeSize(Inf->TargetLetter, VolumeSizeMB);
