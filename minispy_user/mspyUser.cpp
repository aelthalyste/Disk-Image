...
OfflineRestoreCleanDisk(restore_inf* R, int DiskID) {
    ...
    if (BMEX->M.IsOSVolume) {
        if (BMEX->M.DiskType == NAR_DISKTYPE_GPT) {
            ...
            if (NarCreateCleanGPTBootablePartition(DiskID, BMEX->M.VolumeSize / (1024 * 1024), 99, 529, R->TargetLetter)) {
                Result = TRUE;
        ...
        if (BMEX->M.DiskType == NAR_DISKTYPE_MBR) {
            ...
            saglik ocagi
            GPT parsela ona gore disk boyutunu al
            https://github.com/microsoft/Windows-classic-samples/blob/master/Samples/StorageManagement/cpp/StorageManagementApplication.h
            format kodunu test et
...
/*
BOOLEAN
AppendRecoveryToFile(HANDLE File, char Letter, int ClusterSize) {
    ...
    void *P = malloc(2*BufferSize);
    
    VOLUME_DISK_EXTENTS *Ext = (VOLUME_DISK_EXTENTS*)malloc(P);
