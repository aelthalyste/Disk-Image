...
BOOLEAN
SaveSystemPartition(volume_backup_inf* V) {
        ...
        if (Disk != INVALID_HANDLE_VALUE) {
          ...
          if (DeviceIoControl(Disk, IOCTL_DISK_GET_DRIVE_LAYOUT_EX, 0, 0, DL, BS, &T, 0)) {
            ...
            if (DL->PartitionStyle == PARTITION_STYLE_MBR) {
              ...
              std::wstring Path = GenerateSystemPartitionFileName(V->Letter);

              HANDLE Output = CreateFileW(Path.c_str(), GENERIC_WRITE, 0, 0, OPEN_EXISTING, 0, 0);

              if (Output != INVALID_HANDLE_VALUE) {
                void* Buffer = malloc(512);
                DWORD BR = 0;
                if (Buffer != NULL) {
                  if (ReadFile(Disk, Buffer, 512, &BR, 0) && BR == 512) {
                    if (WriteFile(Output, Buffer, 512, &BR, 0) && BR == 512) {
                      // // TODO(Batuhan): Success
                      Result = TRUE;
                    }
                    else {
                      // TODO(Batuhan): writefile failed
                    }
                  }
                  ...
                  // TODO(Batuhan): cant allocate memory
                }
