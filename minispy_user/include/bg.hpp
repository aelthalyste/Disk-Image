...
#if BG_ENABLE_LEAKCHECK && BG_USE_MIMALLOC
#error CANT DEFINE MIMALLOC AND LEAKCHECK TOGETHER, ENABLE ONLY ONE OF THEM OR DISABLE BOTH
#endif
...
#if BG_COMPILER_MSVC
#define BG_UNREACHABLE() __assume(0);
#elif BG_COMPILER_CLANG || BG_COMPILER_GCC
#define BG_UNREACHABLE() __builtin_unreachable();
#endif


#if BG_COMPILER_MSVC 
#define BG_INLINE __forceinline 
// #define BG_INLINE
#else
#define BG_INLINE
#endif
...
bool         bg_init_log_file(const char* file_name);
const char* bg_get_log_file_name();
...
#else
#include "mimalloc.h" 
#define bg_malloc(n)        mi_malloc(n)
#define bg_realloc(p, sz)   mi_realloc(p, sz)
#define bg_free(p)          mi_free(p)
#define bg_calloc(c,s)      mi_calloc(c, s)
#endif
...
static BG_INLINE Pool_Allocator
...
static BG_INLINE Linear_Allocator
...
template<typename T>
void arrdel(Array<T>* arr, u64 indc) {
...
template<typename T>
void arrins(Array<T>* arr, u64 indc, T val) {
    BG_ASSERT(indc < arr->len);
...
template<typename T>
void arrinit(Array<T>* arr, u64 capacity);

template<typename T>
void arrput(Array<T>* arr, T val);

template<typename T>
void arrputn(Array<T>* arr, T* values, u64 n);

template<typename T>
T arrpop(Array<T>* arr);

template<typename T>
void arrfree(Array<T>* arr);

template<typename T>
void arrreserve(Array<T>* arr, u64 n);

template<typename T>
void arr__grow(Array<T>* arr, u64 new_cap);
...
template<typename T>
void arrinit(Array<T>* arr, u64 cap) {
    BG_ASSERT(arr->len == 0);
...
template<typename T>
T* arrputnempty(Array<T>* arr, u64 count) {
...
static BG_INLINE Bg_Random_State bg_init_random(u64 init_state) {
...
void bg_push_leak_context();
void bg_pop_leak_context();





u32 bg_crc32(const void* memory, u64 size);
...
static BG_INLINE char to_lower(char c) {
    if (c >= 'A' && c <= 'Z')
        return (c - 'A') + 'a';
    return c;
}
...
static BG_INLINE BgUtf16 to_lower(BgUtf16 c) {
    return towlower(c);
}

static BG_INLINE void lower_string(char* str) {
    for (; *str != 0; str++) {
        *str = to_lower(*str);
    }
}
...
constexpr char* BG_LOG_PATH = "bg_nar_log_file.txt";
...
static const char* bg__log_file_name = NULL;
...
bool bg_init_log_file(const char* file_name) {
    ...
    if (f) {
        ...
        if (bg__log__internal_file != 0) {
            fclose(bg__log__internal_file);
        }
        bg__log__internal_file = f;
        bg__log_file_name = file_name;
        result = true;
    }

    // well, there is a possible deadlock or silly bug hidden here. we might 
    // free file handle while other thread trying to write to it, but just nevermind.
...
const char* bg_get_log_file_name() {
    return bg__log_file_name;
}

void bg__log(const char* log_prefix, const char* fmt, ...) {
...
u32 bg_crc32(const void* data, u64 len) {
    u64 remaining;
    u32 result = ~((u32)0);
...
u64 bg_murmurhash(const void* key, s32 len, u32 seed) {
...
void bg_push_leak_context() {
#if BG_ENABLE_LEAKCHECK
    return stb_leakcheck_push();
#endif
}

void bg_pop_leak_context() {
#if BG_ENABLE_LEAKCHECK
    return stb_leakcheck_pop();
#endif
}
